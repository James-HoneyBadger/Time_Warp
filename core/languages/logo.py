...
class TwLogoExecutor:
    ...
    def execute_command(self, command):
        ...
        try:
            ...
            # Additional debug: log command parts for shape commands
            if cmd in ["CIRCLE", "DOT", "RECT", "HEADING"]:
                self.interpreter.debug_output(f"Shape command parts: {parts}")
    ...
    def _handle_rect(self, parts):
        """Handle RECT command"""
        try:
            # Handle comma-separated values like "RECT 100, 50"
            if len(parts) >= 2:
                # Parse width and height, handling commas
                width_str = parts[1].replace(',', '')
                height_str = parts[2].replace(',', '') if len(parts) > 2 else parts[1].replace(',', '')
    ...
    def _split_top_level_commands(self, inner):
        ...
        while i < len(tokens):
            ...
            if token in logo_commands:
                ...
                # For commands that expect bracketed blocks (like REPEAT, DEFINE),
                # collect until we have a complete bracketed expression
                if token in ["REPEAT", "DEFINE"]:
                    # Collect the count/identifier
                    while i < len(tokens) and not tokens[i].startswith("["):
                        cmd_parts.append(tokens[i])
                        i += 1
                    # Now collect the bracketed block
                    if i < len(tokens) and tokens[i].startswith("["):
                        ...
                        while i < len(tokens):
                            ...
                            if bracket_depth <= 0:
                                break
                            i += 1
                        # Include all tokens from start to end of bracketed block
                        for j in range(start_i, i + 1):
                            cmd_parts.append(tokens[j])
                        i += 1
                else:
                    # For other commands, collect arguments until we hit another command
                    while i < len(tokens):
