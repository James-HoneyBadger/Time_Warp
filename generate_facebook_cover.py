...
def create_facebook_cover():
    ...
    draw.rectangle([screen_x, screen_y, screen_x + screen_width, screen_y + screen_height],
                  outline=classic_gray, fill=(20, 20, 20), width=3)

    # Screen content
    draw.text((screen_x + 10, screen_y + 10), "10 PRINT \"HELLO\"", font=code_font, fill=neon_green)
    draw.text((screen_x + 10, screen_y + 30), "20 GOTO 10", font=code_font, fill=neon_green)

    # Command prompts
    draw.text((screen_x, screen_y + screen_height + 20), ">_ PILOT", font=code_font, fill=electric_blue)
    draw.text((screen_x, screen_y + screen_height + 40), ">_ BASIC", font=code_font, fill=electric_blue)
    draw.text((screen_x, screen_y + screen_height + 60), ">_ LOGO", font=code_font, fill=electric_blue)
    ...
    for offset in range(3, 0, -1):
        draw.text((logo_x - offset, logo_y - offset), logo_text, font=title_font,
                 fill=(255, 255 - offset*20, 0))  # Orange glow
    ...
    draw.rectangle([term_x, term_y, term_x + term_width, term_y + term_height],
                  fill=(20, 20, 20), outline=electric_blue, width=2)

    # Terminal content
    draw.text((term_x + 10, term_y + 10), "$ timewarp-compiler hello.bas -o hello", font=code_font, fill=neon_green)
    ...
    for i, badge in enumerate(badges):
        ...
        draw.rectangle([badge_x, badge_y + i*25, badge_x + 180, badge_y + i*25 + 20],
                      fill=electric_blue, outline=white, width=1)
        draw.text((badge_x + 5, badge_y + i*25 + 2), badge, font=small_font, fill=white)

    # Add some binary code decoration
    binary_x = width - 50
    binary_y = height - 50
    binary_text = "01010101\n01001000\n01000101\n01001100\n01001100\n01001111"
    draw.text((binary_x, binary_y), binary_text, font=small_font, fill=(neon_green[0], neon_green[1], neon_green[2], 128))
...
def main():
    ...
    rgb_img.paste(cover_img, mask=cover_img.split()[-1] if cover_img.mode == 'RGBA' else None)
