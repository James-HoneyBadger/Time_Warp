    ...
    def _control_iot_device(self):
        ...
        if selection:
            ...
            if "Light" in device_name or "Bulb" in device_type:
                ttk.Button(control_dialog, text="💡 Turn On", command=lambda: self._iot_device_action(device_name, "on")).pack(pady=5)
                ttk.Button(control_dialog, text="🌑 Turn Off", command=lambda: self._iot_device_action(device_name, "off")).pack(pady=5)
                ttk.Button(control_dialog, text="🌈 Change Color", command=lambda: self._iot_device_action(device_name, "color")).pack(pady=5)
            elif "Thermostat" in device_name:
                ttk.Button(control_dialog, text="🔄 Auto Mode", command=lambda: self._iot_device_action(device_name, "auto")).pack(pady=5)
                ttk.Button(control_dialog, text="❄️ Cool Mode", command=lambda: self._iot_device_action(device_name, "cool")).pack(pady=5)
                ttk.Button(control_dialog, text="🔥 Heat Mode", command=lambda: self._iot_device_action(device_name, "heat")).pack(pady=5)
            elif "Camera" in device_name:
                ttk.Button(control_dialog, text="📹 Start Recording", command=lambda: self._iot_device_action(device_name, "record")).pack(pady=5)
                ttk.Button(control_dialog, text="📸 Take Snapshot", command=lambda: self._iot_device_action(device_name, "snapshot")).pack(pady=5)
                ttk.Button(control_dialog, text="🔴 Stop", command=lambda: self._iot_device_action(device_name, "stop")).pack(pady=5)
            elif "Plug" in device_name:
                ttk.Button(control_dialog, text="🔌 Turn On", command=lambda: self._iot_device_action(device_name, "on")).pack(pady=5)
                ttk.Button(control_dialog, text="⚡ Turn Off", command=lambda: self._iot_device_action(device_name, "off")).pack(pady=5)
            
            ttk.Button(control_dialog, text="✅ Close", command=control_dialog.destroy).pack(pady=10)
    
    def _iot_device_action(self, device_name, action):
        """Perform action on IoT device"""
        messagebox.showinfo("Device Action", f"Performed '{action}' on {device_name}")
        self.emit_event('iot_device_action', device_name, action)
