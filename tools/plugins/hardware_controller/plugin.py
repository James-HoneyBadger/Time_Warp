...
class HardwareControllerPlugin(ToolPlugin):
    ...
    def __init__(self, ide_instance, framework):
        ...
        # Hardware state
        self.hardware_state = {"gpio_pins": {}, "sensors": {}, "devices": {}, "automation_rules": []}
    ...
    def create_ui(self, parent_widget) -> tk.Widget:
        ...
        try:
            ...
            ttk.Label(header_frame, text="🔌 Hardware Controller", font=("Arial", 16, "bold")).pack(side=tk.LEFT)

            ttk.Button(header_frame, text="🔄 Refresh All", command=self._refresh_all_hardware).pack(
    ...
    def _setup_gpio_tab(self, notebook):
        ...
        ttk.Radiobutton(
            mode_frame, text="Input", variable=self.pin_mode_var, value="input", command=self._update_pin_mode
        ).pack()
        ttk.Radiobutton(
            mode_frame, text="Output", variable=self.pin_mode_var, value="output", command=self._update_pin_mode
        ).pack()
        ...
        ttk.Radiobutton(
            value_frame, text="LOW (0)", variable=self.pin_value_var, value="0", command=self._update_pin_value
        ).pack()
        ttk.Radiobutton(
            value_frame, text="HIGH (1)", variable=self.pin_value_var, value="1", command=self._update_pin_value
        ).pack()
    ...
    def _setup_sensors_tab(self, notebook):
        ...
        ttk.Button(sensor_controls, text="🔄 Refresh Data", command=self._refresh_sensor_data).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(sensor_controls, text="📊 Start Monitoring", command=self._start_sensor_monitoring).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(sensor_controls, text="⏹️ Stop Monitoring", command=self._stop_sensor_monitoring).pack(
            side=tk.LEFT, padx=2
        )
    ...
    def _setup_automation_tab(self, notebook):
        ...
        ttk.Button(rule_controls, text="❌ Delete Rule", command=self._delete_automation_rule).pack(
            side=tk.LEFT, padx=2
        )
    ...
    def _add_sensor(self):
        ...
        type_combo = ttk.Combobox(
            dialog, textvariable=type_var, values=["DHT22", "DS18B20", "BMP280", "HC-SR04", "PIR", "LDR"]
        )
        ...
        def create_sensor():
            self.sensors_tree.insert(
                "", "end", values=(name_var.get(), type_var.get(), pin_var.get(), "0.0", "units", "Ready")
            )
    ...
    def _control_device(self):
        ...
        if selection:
            ...
            if "LED" in device_name:
                ttk.Button(
                    control_dialog, text="💡 Turn On", command=lambda: self._device_action(device_name, "on")
                ).pack(pady=5)
                ttk.Button(
                    control_dialog, text="🌑 Turn Off", command=lambda: self._device_action(device_name, "off")
                ).pack(pady=5)
            elif "Servo" in device_name:
                ttk.Button(
                    control_dialog, text="↪️ Position 0°", command=lambda: self._device_action(device_name, "pos_0")
                ).pack(pady=5)
                ttk.Button(
                    control_dialog, text="↩️ Position 180°", command=lambda: self._device_action(device_name, "pos_180")
                ).pack(pady=5)
            elif "Buzzer" in device_name:
                ttk.Button(
                    control_dialog, text="🔊 Beep", command=lambda: self._device_action(device_name, "beep")
                ).pack(pady=5)
            elif "Relay" in device_name:
                ttk.Button(
                    control_dialog, text="🔴 Close", command=lambda: self._device_action(device_name, "close")
                ).pack(pady=5)
                ttk.Button(
                    control_dialog, text="🟢 Open", command=lambda: self._device_action(device_name, "open")
                ).pack(pady=5)
